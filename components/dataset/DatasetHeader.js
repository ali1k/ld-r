        ...
        return (
            <div className="ui" ref="datasetHeader">
                <h3 className="ui header">
                    ...
                    {this.props.pivotConstraint && this.props.pivotConstraint.trim() ? <span onClick={this.props.importedEnvState ? ()=>{return 0} : this.props.handleBackToPrevPivotState}><Popup trigger={<div className="ui labeled mini button"><div className="ui mini blue button">{this.props.importedEnvState ? null: <i className="ui zoom out icon"> </i>} {this.props.importedEnvState ? 'constraints': 'Zoom out'}</div><a className="ui basic left pointing label"><i className="ui violet filter icon link "> </i></a></div>} content={this.props.pivotConstraint} wide position='bottom center' /></span> : null}
                </h3>
            </div>
        );
    }
