    ...
    render() {
        ...
        let list = this.props.spec.extendedViewData.map(function(node, index) {
            let llist = node.spec.instances.map(function(instance, index2){
                return (
                    <IndividualDataEdit noFocus="1" key={index + '_' + index2} spec={instance} config={node.config} graphName={self.props.graphName} onDataEdit={self.handleDetailDataEdit.bind(self, node.spec.propertyURI, instance.valueType, instance.dataType)}/>
                );
            });
            return (
